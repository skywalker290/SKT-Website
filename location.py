...
def get_coordinates():
    ...
    with open('city_coordinates.csv', 'w', newline='') as f:
        ...
        for city in tqdm(CITIES, desc="Geocoding"):
            try:
                ...
                tqdm.write(f"Error geocoding {city}: {e}")
    
    return city_coords

def calculate_distances(city_coords):
    print(f"Calculating pairwise distances for {len(city_coords)} cities...")
    
    pairs = list(itertools.combinations(city_coords.keys(), 2))
    
    with open('city_distances.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["City_A", "City_B", "Distance_km"])
        
        for city_a, city_b in tqdm(pairs, desc="Calculating driving distances"):
            ...
            try:
                url = f"https://router.project-osrm.org/route/v1/driving/{lon_a},{lat_a};{lon_b},{lat_b}?overview=false"
                req = urllib.request.Request(url, headers={'User-Agent': 'sri_kela_travels_mapper'})
                ...
                if data.get("routes") and len(data["routes"]) > 0:
                    ...
                    dist_km = round(geodesic(coord_a, coord_b).kilometers, 2)
                    
                writer.writerow([city_a, city_b, dist_km])
                
                # Sleep to respect OSRM's public API rate limits
                time.sleep(0.5) 
            except Exception as e:
                tqdm.write(f"Error calculating driving distance for {city_a} to {city_b}: {e}")
                dist = geodesic(coord_a, coord_b).kilometers
                writer.writerow([city_a, city_b, round(dist, 2)])
            
    print("Distance calculation complete. Saved to city_distances.csv.")
